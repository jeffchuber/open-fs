...
use ax_fuse::{
    block_on, init_runtime, AxFuse, InodeAttr, InodeTable, SearchDir,
};
...
fn test_axfuse_full_lifecycle() {
    ...
    let ax = AxFuse::from_config(config).unwrap();
...
fn test_axfuse_nested_directories() {
    ...
    let paths = ["/workspace/a/file.txt",
...
fn test_axfuse_stat_file_and_directory() {
    ...
    // Stat file
    let file_entry = block_on(async { ax.vfs.stat("/workspace/dir/file.txt").await.unwrap() }).unwrap();
...
fn test_axfuse_append_operations() {
    ...
    // Initial write
    block_on(async {
        ax.vfs.write("/workspace/append.txt", b"Hello").await.unwrap();
    }).unwrap();
...
fn test_axfuse_unicode_filenames() {
    ...
    let unicode_files = ["/workspace/文件.txt",
        "/workspace/файл.txt",
...
fn test_axfuse_overwrite_with_smaller_content() {
    ...
    block_on(async {
        ax.vfs.write("/workspace/shrink.txt", b"small").await.unwrap();
    }).unwrap();
...
fn test_axfuse_handles_concurrent_access_same_file() {
    ...
    let ax = Arc::new(AxFuse::from_config(config).unwrap());

    // Initial write
    {
        let ax = Arc::clone(&ax);
        block_on(async move {
            ax.vfs.write("/workspace/concurrent.txt", b"initial").await.unwrap();
        }).unwrap();
    }
            ...
            for j in 0..100 {
                if j % 2 == 0 {
                    let _ = block_on(async {
                        ax.vfs.read("/workspace/concurrent.txt").await
                    }).unwrap();
                } else {
                    let content = format!("update {} {}", i, j);
                    let _ = block_on(async {
                        ax.vfs.write("/workspace/concurrent.txt", content.as_bytes()).await
                    }).unwrap();
                }
            }
        }));
    ...
    let content = block_on(async move {
        ax.vfs.read("/workspace/concurrent.txt").await.unwrap()
    }).unwrap();
