...
pub fn init_runtime() -> Result<&'static Runtime, FuseError> {
    ...
    match result {
        Ok(rt) => Ok(rt),
        Err(e) => Err(FuseError::Other(format!("Failed to create FUSE async runtime: {}", e))),
    }
}
...
pub fn runtime() -> Result<&'static Runtime, FuseError> {
    match RUNTIME.get() {
        Some(Ok(rt)) => Ok(rt),
        Some(Err(e)) => Err(FuseError::Other(format!("FUSE runtime failed to initialize: {}", e))),
        None => Err(FuseError::Other("FUSE runtime not initialized - call init_runtime first".to_string())),
    }
}
...
impl From<ax_core::VfsError> for FuseError {
    fn from(e: ax_core::VfsError) -> Self {
        match e {
            ax_core::VfsError::NotFound(_) => FuseError::NotFound,
            ax_core::VfsError::ReadOnly(_) => FuseError::ReadOnly,
            ax_core::VfsError::NoMount(_) => FuseError::NotFound,
            ax_core::VfsError::Io(io_err) => FuseError::Io(io_err),
            ax_core::VfsError::Config(msg) => FuseError::Other(msg),
            ax_core::VfsError::Backend(e) => FuseError::Other(e.to_string()),
            ax_core::VfsError::Watch(msg) => FuseError::Other(msg),
            ax_core::VfsError::Indexing(msg) => FuseError::Other(msg),
...
mod tests {
    ...
    fn test_from_io_error_various_kinds() {
        ...
        for kind in kinds {
            ...
            let fuse_err = FuseError::from(io_err);
            assert!(matches!(fuse_err, FuseError::Io(_)), "Failed for kind: {:?}", kind);
        }
    }
    ...
    fn test_from_vfs_error_not_found() {
        let vfs_err = ax_core::VfsError::NotFound("/test".to_string());
    ...
    fn test_from_vfs_error_read_only() {
        let vfs_err = ax_core::VfsError::ReadOnly("/test".to_string());
        let fuse_err = FuseError::from(vfs_err);
    ...
    fn test_from_vfs_error_no_mount() {
        let vfs_err = ax_core::VfsError::NoMount("/test".to_string());
        let fuse_err = FuseError::from(vfs_err);
        assert!(matches!(fuse_err, FuseError::NotFound));
    }
    ...
    fn test_from_vfs_error_config() {
        let vfs_err = ax_core::VfsError::Config("config error".to_string());
        let fuse_err = FuseError::from(vfs_err);
    ...
    fn test_from_vfs_error_io() {
        ...
        let vfs_err = ax_core::VfsError::Io(io_err);
        let fuse_err = FuseError::from(vfs_err);
        assert!(matches!(fuse_err, FuseError::Io(_)));
    }
