...
pub async fn grep(
    ...
    if options.recursive {
        grep_recursive(vfs, path, &re, &mut matches, options.max_matches, options.max_depth).await;
    } else {
...
"#,
    ...
    async fn test_grep_single_file() {
        ...
        let matches = grep(&vfs, "hello", "/workspace/test.txt", &GrepOptions::default())
            .await
            .unwrap();
    ...
    async fn test_grep_directory() {
        ...
        vfs.write("/workspace/b.txt", b"goodbye world").await.unwrap();
