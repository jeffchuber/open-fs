...
use ax_core::{Backend, BackendError, ChromaStore, Entry, QueryResult, SparseEmbedding};
...
impl Backend for ChromaHttpBackend {
    ...
    async fn write(&self, path: &str, content: &[u8]) -> Result<(), BackendError> {
        ...
        ChromaStore::upsert(self, path, &text, None, None, None).await
    }
    ...
    async fn append(&self, path: &str, content: &[u8]) -> Result<(), BackendError> {
        ...
        ChromaStore::upsert(self, path, &new_content, None, None, None).await
    }
...
mod tests {
    ...
    #[test]
    fn test_path_to_id() {
        assert_eq!(ChromaHttpBackend::path_to_id("/workspace/test.txt"), "workspace_test.txt");
