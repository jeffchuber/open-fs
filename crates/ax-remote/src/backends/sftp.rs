    ...
    pub async fn new(config: SftpConfig) -> Result<Self, BackendError> {
        ...
        let authenticated = if let Some(ref key_path) = config.private_key {
            ...
            session
                ...
                .await
                .map_err(|e| BackendError::Other(format!("Public key authentication failed: {}", e)))?
        } else if let Some(ref password) = config.password {
            session
                .authenticate_password(&config.username, password.expose())
                .await
                .map_err(|e| BackendError::Other(format!("Password authentication failed: {}", e)))?
        } else {
        ...
        if !authenticated {
            return Err(BackendError::PermissionDenied("SSH authentication failed".to_string()));
        }
