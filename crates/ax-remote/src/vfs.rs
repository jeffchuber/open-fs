...
async fn create_backend(
    ...
    match backend_config {
        ...
        BackendConfig::WebDav(webdav_config) => {
            #[cfg(feature = "webdav")]
            {
                let backend = backends::WebDavBackend::new(backends::WebDavConfig {
                    url: webdav_config.url.clone(),
            ...
            #[cfg(not(feature = "webdav"))]
            {
                let _ = webdav_config;
                Err(VfsError::Config(
                    "WebDAV backend requires the 'webdav' feature flag".to_string(),
                ))
            }
        }
        BackendConfig::Sftp(sftp_config) => {
            #[cfg(feature = "sftp")]
            {
                let backend = backends::SftpBackend::new(backends::SftpConfig {
                    host: sftp_config.host.clone(),
                    port: sftp_config.port,
                    username: sftp_config.username.clone(),
                    password: sftp_config.password.clone(),
                    private_key: sftp_config.private_key.clone(),
                    root: sftp_config.root.clone().unwrap_or_else(|| "/".to_string()),
                })
                .await
                .map_err(VfsError::from)?;
                Ok(Arc::new(backend) as Arc<dyn Backend>)
            ...
            #[cfg(not(feature = "sftp"))]
            {
                let _ = sftp_config;
                Err(VfsError::Config(
                    "SFTP backend requires the 'sftp' feature flag".to_string(),
                ))
            }
        }
        BackendConfig::Gcs(gcs_config) => {
            #[cfg(feature = "gcs")]
            {
                let backend = backends::GcsBackend::new(backends::GcsConfig {
                    bucket: gcs_config.bucket.clone(),
                    prefix: gcs_config.prefix.clone(),
                    credentials_file: gcs_config.credentials_file.clone(),
                })
                .map_err(VfsError::from)?;
                Ok(Arc::new(backend) as Arc<dyn Backend>)
            }
            #[cfg(not(feature = "gcs"))]
            {
                let _ = gcs_config;
                Err(VfsError::Config(
                    "GCS backend requires the 'gcs' feature flag".to_string(),
                ))
            }
        }
        BackendConfig::AzureBlob(azure_config) => {
            #[cfg(feature = "azure")]
            {
                let backend = backends::AzureBlobBackend::new(backends::AzureBlobConfig {
                    account: azure_config.account.clone(),
                    container: azure_config.container.clone(),
                    access_key: azure_config.access_key.clone(),
                    prefix: azure_config.prefix.clone(),
                })
                .map_err(VfsError::from)?;
                Ok(Arc::new(backend) as Arc<dyn Backend>)
            }
            #[cfg(not(feature = "azure"))]
            {
                let _ = azure_config;
                Err(VfsError::Config(
                    "Azure Blob backend requires the 'azure' feature flag".to_string(),
                ))
            }
        }
        _ => {
