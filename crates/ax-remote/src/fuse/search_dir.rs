...
impl SearchDir {
    ...
    pub fn extract_query(path: &str) -> Option<String> {
        ...
        urlencoding::decode(query_encoded).ok().map(|s| s.into_owned())
    }
    ...
    pub fn readdir(&self, path: &str) -> Option<Vec<(u64, String, InodeKind)>> {
        if Self::is_search_root(path) {
            return Some(vec![(VIRTUAL_INO_BASE + 1, "query".to_string(), InodeKind::Directory)]);
        }
    ...
    pub fn create_result_entries(
        ...
        let content = if results.is_empty() {
            ...
            for (i, (source_path, snippet, score, start_line, end_line)) in results.iter().enumerate()
            {
                if i > 0 {
                    out.push_str("\n\n");
                }
