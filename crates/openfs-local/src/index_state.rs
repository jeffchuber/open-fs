...
impl IndexState {
    ...
    pub fn compute_delta(&self, current_files: &[FileInfo]) -> DeltaResult {
        ...
        let current_set: HashMap<&str, &FileInfo> =
    ...
    pub fn reconcile(&self, current_files: &[FileInfo]) -> ReconcileResult {
        ...
        for path in self.files.keys() {
            if !current_set.contains_key(path.as_str()) {
                result
                    .actions
                    .push(ReconcileAction::RemoveOrphan { path: path.clone() });
    ...
    pub fn default_path(base_dir: &Path) -> PathBuf {
        base_dir.join(".openfs-index-state.json")
    }
...
mod tests {
    ...
    fn test_default_path() {
        ...
        assert_eq!(path, PathBuf::from("/workspace/.openfs-index-state.json"));
    }
}
