...
impl OpenFsCore {
    fn materialize_query_results(&self, query_path: &str) -> Result<(), FsOpError> {
        ...
        let grep_result = block_on(async {
            let opts = openfs_remote::GrepOptions {
                recursive: true,
                max_matches: 200,
                max_depth: 20,
            };
            openfs_remote::grep(&vfs, &query, "/", &opts).await
        })?;
    ...
    pub fn do_lookup(&self, parent: u64, name: &str) -> Result<InodeAttr, FsOpError> {
        ...
        match result {
            Ok(entry) => {
                let ino =
                    self.inodes
                        .get_or_create(&child_path, entry.is_dir, entry.size.unwrap_or(0));
                self.inodes
                    .get_attr(ino)
                    .ok_or(FsOpError::Io("inode missing after creation".to_string()))
            }
    ...
    pub fn do_getattr(&self, ino: u64) -> Result<InodeAttr, FsOpError> {
        ...
        match result {
            Ok(entry) => {
                self.inodes
                    .update_attr(ino, entry.size.unwrap_or(0), entry.is_dir);
    ...
    pub fn do_write(&self, ino: u64, offset: i64, data: &[u8]) -> Result<u32, FsOpError> {
        ...
        let result = block_on(async {
            if offset == 0 {
                ...
                let existing = match vfs.read(&path).await {
                    Ok(content) => content,
                    Err(openfs_core::VfsError::NotFound(_)) => Vec::new(),
                    Err(e) => return Err(e),
                };
        ...
        match result {
            ...
            Err(openfs_core::VfsError::ReadOnly(_)) => Err(FsOpError::ReadOnly),
            Err(e) => Err(FsOpError::Io(e.to_string())),
        }
    }
    ...
    pub fn do_readdir(&self, ino: u64) -> Result<ReadDirResult, FsOpError> {
        ...
        if SearchDir::is_search_path(&path) {
            if let Some(entries) = self.search_dir.readdir(&path) {
                let parent_ino = if path == SEARCH_DIR_PATH {
                    ROOT_INO
                } else {
                    ino
                };
                return Ok(ReadDirResult {
                    ...
                    entries: entries
                        .into_iter()
                        .map(|(entry_ino, name, kind)| DirEntry {
                            ino: entry_ino,
                            name,
                            kind,
                        })
        ...
        match result {
            Ok(vfs_entries) => {
                ...
                let entries: Vec<DirEntry> = vfs_entries
                    .into_iter()
                    .map(|entry| {
    ...
    pub fn do_unlink(&self, parent: u64, name: &str) -> Result<(), FsOpError> {
        ...
        match result {
            ...
            Err(openfs_core::VfsError::ReadOnly(_)) => Err(FsOpError::ReadOnly),
            Err(openfs_core::VfsError::NotFound(_)) => Err(FsOpError::NotFound),
            Err(e) => Err(FsOpError::Io(e.to_string())),
        }
    }
    ...
    pub fn do_rmdir(&self, parent: u64, name: &str) -> Result<(), FsOpError> {
        ...
        match list_result {
            Ok(entries) => {
                let non_placeholder: Vec<_> =
                    entries.iter().filter(|e| e.name != ".axkeep").collect();
    ...
    pub fn mount(self, mountpoint: &Path) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let options = vec![
            MountOption::FSName("openfs".to_string()),
            MountOption::AutoUnmount,
            MountOption::AllowOther,
            MountOption::DefaultPermissions,
        ];

        info!("Mounting OpenFS at {:?}", mountpoint);
        fuser::mount2(UnixFuse(self), mountpoint, &options)?;
        info!("OpenFS unmounted");
    ...
    pub fn mount_foreground(self, mountpoint: &Path) -> Result<(), Box<dyn std::error::Error>> {
        ...
        info!("Mounting OpenFS at {:?} (foreground)", mountpoint);
        fuser::mount2(UnixFuse(self), mountpoint, &options)?;
...
"#,
    ...
    fn test_core_from_config() {
        ...
        let core = OpenFsCore::from_config(config);
        assert!(core.is_ok());
    }
    ...
    fn test_core_has_root_inode() {
        let temp_dir = TempDir::new().unwrap();
        let config = make_test_config(temp_dir.path().to_str().unwrap());
        let core = OpenFsCore::from_config(config).unwrap();
        assert_eq!(core.get_path(ROOT_INO), Some("/".to_string()));
    }

    #[test]
    fn test_child_path_from_root() {
        assert_eq!(OpenFsCore::child_path("/", "file.txt"), "/file.txt");
    }

    #[test]
    fn test_child_path_from_subdir() {
        assert_eq!(
            OpenFsCore::child_path("/workspace", "file.txt"),
            "/workspace/file.txt"
        );
    }
    ...
    fn test_core_vfs_read_write() {
        ...
        let content =
            block_on(async { core.vfs.read("/workspace/test.txt").await.unwrap() }).unwrap();
    ...
    fn test_core_vfs_list() {
        ...
        block_on(async {
            core.vfs
                .write("/workspace/file1.txt", b"content1")
                .await
                .unwrap();
            core.vfs
                .write("/workspace/file2.txt", b"content2")
                .await
                .unwrap();
        })
        .unwrap();

        let entries = block_on(async { core.vfs.list("/workspace").await.unwrap() }).unwrap();
    ...
    fn test_core_vfs_exists() {
        ...
        let exists =
            block_on(async { core.vfs.exists("/workspace/test.txt").await.unwrap() }).unwrap();
        assert!(!exists);

        block_on(async {
            core.vfs
                .write("/workspace/test.txt", b"content")
                .await
                .unwrap()
        })
        .unwrap();
    ...
    fn test_core_vfs_stat() {
        ...
        block_on(async {
            core.vfs
                .write("/workspace/test.txt", b"hello")
                .await
                .unwrap()
        })
        .unwrap();
        ...
        let entry =
            block_on(async { core.vfs.stat("/workspace/test.txt").await.unwrap() }).unwrap();
    ...
    fn test_core_handles_special_characters() {
        ...
        block_on(async {
            core.vfs
                .write("/workspace/file with spaces.txt", b"content")
                .await
                .unwrap();
        })
        .unwrap();
        ...
        let content = block_on(async {
            core.vfs
                .read("/workspace/file with spaces.txt")
                .await
                .unwrap()
        })
        .unwrap();
    ...
    fn test_core_handles_large_file() {
        ...
        block_on(async {
            core.vfs
                .write("/workspace/large.bin", &large_content)
                .await
                .unwrap()
        })
        .unwrap();
        ...
        let content =
            block_on(async { core.vfs.read("/workspace/large.bin").await.unwrap() }).unwrap();
    ...
    fn test_core_handles_binary_content() {
        ...
        block_on(async {
            core.vfs
                .write("/workspace/binary.bin", &binary_content)
                .await
                .unwrap()
        })
        .unwrap();
        ...
        let content =
            block_on(async { core.vfs.read("/workspace/binary.bin").await.unwrap() }).unwrap();
    ...
    fn test_core_handles_empty_file() {
        ...
        let content =
            block_on(async { core.vfs.read("/workspace/empty.txt").await.unwrap() }).unwrap();
    ...
    fn test_core_concurrent_reads() {
        ...
        let temp_dir = TempDir::new().unwrap();
        let config = make_test_config(temp_dir.path().to_str().unwrap());
        let core = Arc::new(OpenFsCore::from_config(config).unwrap());

        {
            let core = Arc::clone(&core);
            block_on(async move {
                core.vfs
                    .write("/workspace/shared.txt", b"shared content")
                    .await
                    .unwrap();
            })
            .unwrap();
        ...
        let handles: Vec<_> = (0..10)
            .map(|_| {
                let core = Arc::clone(&core);
                thread::spawn(move || {
                    block_on(async move { core.vfs.read("/workspace/shared.txt").await.unwrap() })
                        .unwrap()
                })
            })
            .collect();
...
"#,
    ...
    fn test_core_multi_mount() {
        ...
        block_on(async {
            core.vfs
                .write("/workspace/file.txt", b"content")
                .await
                .unwrap()
        })
        .unwrap();
        ...
        let exists =
            block_on(async { core.vfs.exists("/workspace/file.txt").await.unwrap() }).unwrap();
        assert!(exists);
    ...
    fn test_core_read_only_mount() {
        ...
        let content =
            block_on(async { core.vfs.read("/docs/existing.txt").await.unwrap() }).unwrap();
        assert_eq!(content, b"content");
