...
use crate::common::{inode_attr_to_file_attr, OpenFsCore, FsOpError};
use crate::inode::{InodeAttr, InodeKind, VIRTUAL_INO_BASE};

/// Unix FUSE filesystem wrapper around `OpenFsCore`.
pub struct UnixFuse(pub OpenFsCore);
...
mod tests {
    ...
    fn test_error_to_errno_mapping() {
        ...
        assert_eq!(
            UnixFuse::error_to_errno(&FsOpError::InvalidArg),
            libc::EINVAL
        );
        assert_eq!(
            UnixFuse::error_to_errno(&FsOpError::NotEmpty),
            libc::ENOTEMPTY
        );
        assert_eq!(
            UnixFuse::error_to_errno(&FsOpError::Io("test".to_string())),
            libc::EIO
        );
        assert_eq!(
            UnixFuse::error_to_errno(&FsOpError::NotSymlink),
            libc::EINVAL
        );
