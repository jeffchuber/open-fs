...
//!
//! This crate provides a FUSE (Filesystem in Userspace) interface to the AX
...
//! # Architecture
//!
//! The crate is split into platform-neutral and platform-specific modules:
//! - `common` — `OpenFsCore` struct with all VFS interaction logic
...
//!
//! // Mount AX VFS
//! let ax = OpenFsFuse::from_config("openfs.yaml")?;
//! ax.mount("/mnt/ax")?;
//! ```
...
pub use common::{OpenFsCore, DirEntry, FsOpError, ReadDirResult};
...
///
/// This is a type alias for `OpenFsCore`, which contains all the platform-neutral
/// filesystem logic. Use `mount()` or `mount_foreground()` to mount it.
pub type OpenFsFuse = OpenFsCore;
