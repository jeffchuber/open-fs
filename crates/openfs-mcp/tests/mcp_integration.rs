...
use openfs_mcp::{McpHandler, McpServer, INVALID_PARAMS, METHOD_NOT_FOUND, PARSE_ERROR};
...
async fn test_full_session_lifecycle() {
    ...
    assert!(tool_names.contains(&"openfs_read"));
    assert!(tool_names.contains(&"openfs_write"));
    assert!(tool_names.contains(&"openfs_ls"));
    assert!(tool_names.contains(&"openfs_exists"));
    assert!(tool_names.contains(&"openfs_rename"));

    // 3. Write a file
    let resp = server
        .handle_message(
            r#"{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"openfs_write","arguments":{"path":"/workspace/session.txt","content":"session data"}}}"#,
        )
        .await
        .unwrap();
    ...
    let resp = server
        .handle_message(
            r#"{"jsonrpc":"2.0","id":4,"method":"tools/call","params":{"name":"openfs_read","arguments":{"path":"/workspace/session.txt"}}}"#,
        )
        .await
        .unwrap();
    let result = resp.result.unwrap();
    ...
    assert_eq!(parsed["content"], "session data");

    // 5. Verify via stat
    let resp = server
        .handle_message(
            r#"{"jsonrpc":"2.0","id":5,"method":"tools/call","params":{"name":"openfs_stat","arguments":{"path":"/workspace/session.txt"}}}"#,
        )
        .await
        .unwrap();
    let result = resp.result.unwrap();
...
async fn test_write_delete_verify_flow() {
    ...
    server
        .handle_message(
            r#"{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"openfs_write","arguments":{"path":"/workspace/ephemeral.txt","content":"temp data"}}}"#,
        )
        .await
        .unwrap();
    ...
    let resp = server
        .handle_message(
            r#"{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"openfs_delete","arguments":{"path":"/workspace/ephemeral.txt"}}}"#,
        )
        .await
        .unwrap();
    assert!(resp.error.is_none());
    ...
    let resp = server
        .handle_message(
            r#"{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"openfs_read","arguments":{"path":"/workspace/ephemeral.txt"}}}"#,
        )
        .await
        .unwrap();
    let result = resp.result.unwrap();
...
async fn test_grep_flow() {
    ...
    let resp = server
        .handle_message(
            r#"{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"openfs_grep","arguments":{"pattern":"find me","path":"/workspace"}}}"#,
        )
        .await
        .unwrap();
    let result = resp.result.unwrap();
...
async fn test_ls_flow() {
    ...
    let resp = server
        .handle_message(
            r#"{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"openfs_ls","arguments":{"path":"/workspace"}}}"#,
        )
        .await
        .unwrap();
    let result = resp.result.unwrap();
...
async fn test_jsonrpc_compliance() {
    ...
    for msg in messages {
        ...
        assert_eq!(
            resp.jsonrpc, "2.0",
            "Response missing jsonrpc 2.0 for: {}",
            msg
        );
    }
