...
pub struct Oracle {
    /// Per-agent private file state in slots:
    /// `slot = agent_id * 2 + mount_offset`, where
    /// `mount_offset=0` for work and `mount_offset=1` for indexed.
    ...
    /// Whether agents 1/2 share one indexed namespace (remote0).
    indexed_shared_12: bool,
}
...
impl Oracle {
    pub fn new() -> Self {
        Self::new_with_indexed_shared_12(false)
    }

    pub fn new_with_indexed_shared_12(indexed_shared_12: bool) -> Self {
        Oracle {
            // Reserve slots for up to 4 agents.
            agent_files: vec![HashMap::new(); 8],
    ...
    fn private_index(&self, agent_id: usize, mount: MountId) -> usize {
        let offset = match mount {
            MountId::Work => 0,
            MountId::Indexed => 1,
            _ => panic!("private_index called with shared mount"),
        };
        if mount == MountId::Indexed && self.indexed_shared_12 && (1..=2).contains(&agent_id) {
            return 3;
        }
        agent_id * 2 + offset
    }

    pub fn indexed_shared_12(&self) -> bool {
        self.indexed_shared_12
    }
        ...
        match op {
            Op::Write {
                ...
                if mount.is_shared() {
                    ...
                    self.shared_write_last_writer.insert(path.clone(), agent_id);
                    return;
                }
                let idx = self.private_index(agent_id, *mount);
