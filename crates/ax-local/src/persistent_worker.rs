...
impl PersistentIndexWorker {
    ...
    pub async fn send(&self, event: PersistentEvent) -> Result<(), VfsError> {
        self.sender.send(event).await.map_err(|e| {
            VfsError::Indexing(format!("Failed to send event: {}", e))
        })
    }
        ...
        B: Backend + 'static,
    {
        ...
        for item in items {
            ...
            debug!("Processing: {} ({})", item.path, if deleted { "delete" } else { "index" });

            match indexer.handle_change(backend.as_ref(), &item.path, deleted).await {
                Ok(()) => {
...
mod tests {
    ...
    async fn test_persistent_worker_basic() {
        ...
        let (worker, handle) = PersistentIndexWorker::spawn(
            ...
            state_path,
            queue_path,
            WorkQueueConfig { debounce_secs: 0, ..Default::default() },
            16,
        ).unwrap();

        worker.send(PersistentEvent::FileChanged {
