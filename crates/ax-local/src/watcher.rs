...
impl WatchEngine {
    ...
    pub fn watch_path(&mut self, fs_root: &Path) -> Result<(), VfsError> {
        ...
        let mut watcher = if let Some(poll_interval) = poll_interval_from_env() {
            let config = Config::default()
                .with_poll_interval(poll_interval)
                .with_compare_contents(true);
            WatcherImpl::Poll(
                PollWatcher::new(handler, config)
                    .map_err(|e| VfsError::Watch(format!("Failed to create poll watcher: {}", e)))?,
            )
        } else {
            let config = Config::default();
            WatcherImpl::Recommended(
                RecommendedWatcher::new(handler, config).map_err(|e| {
                    VfsError::Watch(format!("Failed to create watcher: {}", e))
                })?,
            )
        };
...
fn poll_interval_from_env() -> Option<Duration> {
    let value = std::env::var("AX_WATCH_POLL_INTERVAL_MS").ok()?;
...
mod tests {
    ...
    fn ensure_polling() {
        ...
        INIT.call_once(|| {
            std::env::set_var("AX_WATCH_POLL_INTERVAL_MS", "50");
        });
    }
