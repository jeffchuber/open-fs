...
impl WorkQueue {
    ...
    pub fn fail(&self, id: i64, error: &str) -> Result<(), String> {
        ...
        if new_attempts >= self.config.max_retries {
            ...
            warn!("Moved item {} to dead letter after {} attempts", id, new_attempts);
        } else {
            ...
            debug!("Scheduled retry for item {} in {}s (attempt {})", id, backoff, new_attempts);
        }
...
mod tests {
    ...
    fn test_queue_item_status_roundtrip() {
        assert_eq!(QueueItemStatus::from_str(QueueItemStatus::Pending.as_str()), QueueItemStatus::Pending);
        assert_eq!(QueueItemStatus::from_str(QueueItemStatus::Processing.as_str()), QueueItemStatus::Processing);
        assert_eq!(QueueItemStatus::from_str(QueueItemStatus::DeadLetter.as_str()), QueueItemStatus::DeadLetter);
    }

    #[test]
    fn test_event_type_roundtrip() {
        assert_eq!(QueueEventType::from_str(QueueEventType::Changed.as_str()), QueueEventType::Changed);
        assert_eq!(QueueEventType::from_str(QueueEventType::Deleted.as_str()), QueueEventType::Deleted);
    }
}
