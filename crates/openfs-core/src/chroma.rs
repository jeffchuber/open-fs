...
/// Lightweight embedding trait for use in write paths.
///
/// This is intentionally minimal (single-text only) to keep it in openfs-core
/// without pulling in the full indexing pipeline. Implementations live in
/// openfs-local and adapt the richer `Embedder` trait to this interface.
#[async_trait]
pub trait TextEmbedder: Send + Sync + 'static {
    /// Embed a single piece of text, returning a dense vector.
