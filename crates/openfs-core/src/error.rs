...
pub enum BackendError {
    ...
    /// Conditional write precondition failed (CAS/OCC mismatch).
    #[error("Precondition failed for path '{path}': expected={expected}, actual={actual}")]
    PreconditionFailed {
        path: String,
        expected: String,
        actual: String,
...
pub enum VfsError {
    ...
    #[error("No mount found for path '{0}'. Check your openfs.yaml mounts configuration.")]
    NoMount(String),

    /// Attempted to write to a read-only mount.
    #[error(
        "Mount is read-only: {0}. Remove 'read_only: true' from the mount config to enable writes."
    )]
    ReadOnly(String),
...
mod tests {
    ...
    fn test_vfs_from_config_error() {
        let config_err = openfs_config::ConfigError::InvalidConfig("bad config".to_string());
