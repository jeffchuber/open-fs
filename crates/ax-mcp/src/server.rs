        ...
        match request.method.as_str() {
            "initialize" => {
                ...
                match serde_json::to_value(result) {
                    ...
                    Err(e) => Some(JsonRpcResponse::error(id, INTERNAL_ERROR, format!("Serialization error: {}", e))),
                }
            }
            "tools/list" => {
                ...
                let result = match tokio::time::timeout(
                    ...
                    Err(_) => {
                        return Some(JsonRpcResponse::error(
                            id,
                            INTERNAL_ERROR,
                            format!("Tool '{}' timed out after {}s", params.name, tool_timeout.as_secs()),
                        ));
                    }
                };
                match serde_json::to_value(result) {
...
"#,
    ...
    async fn test_tools_call_read() {
        ...
        assert!(resp.result.is_some());
        let result = resp.result.unwrap();
        assert_eq!(result["content"][0]["text"], "hello from file");
    }
    ...
    async fn test_tools_call_write_and_read() {
        ...
        // Write
        let msg = r#"{"jsonrpc":"2.0","id":4,"method":"tools/call","params":{"name":"ax_write","arguments":{"path":"/workspace/new.txt","content":"written via mcp"}}}"#;
        let resp = server.handle_message(msg).await.unwrap();
        assert!(resp.error.is_none());

        // Read back
        let msg = r#"{"jsonrpc":"2.0","id":5,"method":"tools/call","params":{"name":"ax_read","arguments":{"path":"/workspace/new.txt"}}}"#;
        let resp = server.handle_message(msg).await.unwrap();
        let result = resp.result.unwrap();
        assert_eq!(result["content"][0]["text"], "written via mcp");
    }
    ...
    async fn test_tools_call_ls() {
        ...
        let msg = r#"{"jsonrpc":"2.0","id":8,"method":"tools/call","params":{"name":"ax_ls","arguments":{"path":"/workspace"}}}"#;
        let resp = server.handle_message(msg).await.unwrap();
        let result = resp.result.unwrap();
