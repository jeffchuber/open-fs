...
///
/// This backend stores files in Azure Blob Storage containers.
...
impl AzureBlobBackend {
    /// Create a new Azure Blob Storage backend with the given configuration.
    ...
    /// Convert a VFS path to an Azure blob name.
    ...
    /// Convert an Azure blob name to a VFS path.
    ...
    /// URL for a specific blob.
    fn blob_url(&self, key: &str) -> String {
        format!("{}/{}", self.base_url(), key)
    }
}

/// Azure list blobs response.
#[derive(Deserialize)]
...
impl Backend for AzureBlobBackend {
    ...
    async fn list(&self, path: &str) -> Result<Vec<Entry>, BackendError> {
        ...
        loop {
            let mut url = format!(
                "{}?restype=container&comp=list&prefix={}&delimiter=/",
                self.base_url(),
            ...
            if let Some(blobs) = &list_response.blobs {
                ...
                for prefix in &blobs.blob_prefix {
                    ...
                    if !dir_name.is_empty() {
                        entries.push(Entry::dir(
                            self.key_to_path(prefix.name.trim_end_matches('/')),
                            dir_name.to_string(),
                            None,
                        ));
                    }
                ...
                // Add files from blobs
                for blob in &blobs.blob {
                    if blob.name == prefix_key || blob.name.ends_with('/') {
                        continue;
                    }
                    ...
                    let size = blob
                        .properties
                        .as_ref()
                        .and_then(|p| p.content_length.as_ref())
                        .and_then(|s| s.parse::<u64>().ok())
                        .unwrap_or(0);
                    let modified = blob
                        .properties
                        .as_ref()
                        .and_then(|p| p.last_modified.as_ref())
                        .and_then(|s| DateTime::parse_from_rfc2822(s).ok())
                        .map(|dt| dt.with_timezone(&Utc));

                    entries.push(Entry::file(
                        self.key_to_path(&blob.name),
                        name,
                        size,
                        modified,
                    ));
...
mod tests {
    ...
    fn test_filename() {
        ...
        assert_eq!(AzureBlobBackend::filename("file.txt"), "file.txt");
        assert_eq!(AzureBlobBackend::filename("/a/b/c.rs"), "c.rs");
    }
