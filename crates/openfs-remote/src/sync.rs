...
use openfs_config::BackoffStrategy;
...
impl SyncEngine {
    ...
    pub async fn queue_delete(&self, path: String) -> Result<(), VfsError> {
        if let Some(wal) = &self.wal {
            ...
            // Deletes are applied synchronously by the caller before queueing.
            // Clear any stale outbox entries for this path rather than enqueueing
            // a durable delete item that would otherwise remain pending forever.
            wal.complete_outbox_for_path(&path)
                .map_err(|e| VfsError::Config(format!("Outbox clear failed for delete: {}", e)))?;
        }
    ...
    async fn ensure_started(&self) -> Result<(), VfsError> {
        if self.config.mode != SyncMode::WriteBack {
            return Err(VfsError::Config(
                "Sync engine not in write-back mode".to_string(),
            ));
        }
...
fn is_transient_error(err: &VfsError) -> bool {
    match err {
        VfsError::Backend(boxed) => {
            if let Some(backend_err) = boxed.downcast_ref::<openfs_core::BackendError>() {
...
mod tests {
    ...
    async fn test_sync_engine_write_back() {
        ...
        engine
            .queue_write("/a.txt".to_string(), b"aaa".to_vec())
            .await
            .unwrap();
        engine
            .queue_write("/b.txt".to_string(), b"bbb".to_vec())
            .await
            .unwrap();
    ...
    fn test_compute_backoff_fixed() {
        ...
        assert_eq!(
            compute_backoff(base, 0, BackoffStrategy::Fixed),
            Duration::from_secs(1)
        );
        assert_eq!(
            compute_backoff(base, 1, BackoffStrategy::Fixed),
            Duration::from_secs(1)
        );
    }
    ...
    fn test_compute_backoff_exponential() {
        let base = Duration::from_secs(1);
        assert_eq!(
            compute_backoff(base, 0, BackoffStrategy::Exponential),
            Duration::from_secs(1)
        );
        assert_eq!(
            compute_backoff(base, 1, BackoffStrategy::Exponential),
            Duration::from_secs(2)
        );
        assert_eq!(
            compute_backoff(base, 2, BackoffStrategy::Exponential),
