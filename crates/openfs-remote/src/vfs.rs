...
async fn apply_outbox_entry(
    ...
    match entry.op_type {
        ...
        WalOpType::Append => {
            ...
            backend
                .append(&entry.path, &content)
                .await
                .map_err(VfsError::from)
        }
        WalOpType::Delete => match backend.delete(&entry.path).await {
            Ok(()) => Ok(()),
...
async fn replay_outbox_entries(
    ...
    loop {
        ...
        for entry in ready {
            ...
            wal.mark_processing(entry.id).map_err(|e| {
                VfsError::Config(format!("Failed to mark outbox processing: {}", e))
            })?;

            match apply_outbox_entry(backend.clone(), &entry).await {
                Ok(()) => {
                    wal.complete_outbox(entry.id).map_err(|e| {
                        VfsError::Config(format!("Failed to complete outbox entry: {}", e))
                    })?;
                    applied += 1;
                }
                Err(e) => {
...
async fn create_backend(
    ...
    match backend_config {
        BackendConfig::Fs(fs_config) => {
            let fs_backend = backends::FsBackend::new(&fs_config.root).map_err(VfsError::from)?;
            Ok(Arc::new(fs_backend))
        }
        BackendConfig::Memory(_) => Ok(Arc::new(backends::MemoryBackend::new())),
            ...
            #[cfg(feature = "postgres")]
            {
                let backend = backends::PostgresBackend::new(backends::PostgresConfig {
                    ...
                    table_name: pg_config
                        .table_name
                        .clone()
                        .unwrap_or_else(|| "openfs_files".to_string()),
        ...
        BackendConfig::Chroma(chroma_config) => {
            ...
            let tenant = chroma_config.tenant.as_deref();
            let database = chroma_config.database.as_deref();
        ...
        _ => Err(VfsError::Config(format!(
            "Unsupported backend type for '{}'",
            name
        ))),
    }
}
...
impl Vfs {
    ...
    pub async fn read_with_cas_token(
        &self,
        path: &str,
    ) -> Result<(Vec<u8>, Option<String>), VfsError> {
        let (backend, relative, _) = self.router.resolve(path)?;
        debug!(relative = %relative, "resolved path (CAS read)");
        backend
            .read_with_cas_token(&relative)
            .await
            .map_err(VfsError::from)
    }
    ...
    pub async fn compare_and_swap(
        ...
        debug!(relative = %relative, "resolved path (CAS write)");
        backend
            .compare_and_swap(&relative, expected, content)
            .await
            .map_err(VfsError::from)
    }
    ...
    pub async fn rename(&self, from: &str, to: &str) -> Result<(), VfsError> {
        ...
        if std::ptr::eq(
            ...
            let content = from_backend
                .read(&from_relative)
                .await
                .map_err(VfsError::from)?;
            to_backend
                .write(&to_relative, &content)
                .await
                .map_err(VfsError::from)?;
            from_backend
                .delete(&from_relative)
                .await
                .map_err(VfsError::from)?;
            Ok(())
        }
    ...
    pub async fn read_batch(&self, paths: &[&str]) -> Vec<Result<Vec<u8>, VfsError>> {
        let futures: Vec<_> = paths.iter().map(|p| self.read(p)).collect();
        futures::future::join_all(futures).await
    }

    /// Write multiple files concurrently.
    pub async fn write_batch(&self, files: &[(&str, &[u8])]) -> Vec<Result<(), VfsError>> {
        let futures: Vec<_> = files.iter().map(|(p, c)| self.write(p, c)).collect();
        futures::future::join_all(futures).await
    }

    /// Delete multiple files concurrently.
    pub async fn delete_batch(&self, paths: &[&str]) -> Vec<Result<(), VfsError>> {
        let futures: Vec<_> = paths.iter().map(|p| self.delete(p)).collect();
        futures::future::join_all(futures).await
    }

    /// Prefetch paths into cache by reading them.
    pub async fn prefetch(&self, paths: &[&str]) -> (usize, usize) {
        let futures: Vec<_> = paths.iter().map(|p| self.read(p)).collect();
    ...
    /// Aggregate cache stats across all mounts.
    pub async fn cache_stats(&self) -> openfs_core::CacheStats {
        let mut aggregated = openfs_core::CacheStats::default();
        for runtime in &self.mount_runtimes {
            let stats = runtime.cached_backend.cache_stats().await;
    ...
    pub async fn sync_statuses(&self) -> Result<Vec<MountSyncStatus>, VfsError> {
        ...
        for runtime in &self.mount_runtimes {
            ...
            let outbox = runtime
                ...
                .wal()
                .map(|wal| {
                    wal.outbox_stats().map_err(|e| {
                        VfsError::Config(format!("Failed to read outbox stats: {}", e))
                    })
                })
                .transpose()?;
...
"#,
    ...
    async fn test_vfs_flush_write_back() {
        ...
        vfs.write("/wb_flush_test/file.txt", b"flush me")
            .await
            .unwrap();
