...
impl WriteAheadLog {
    ...
    pub fn checkpoint(&self) -> Result<usize, String> {
        ...
        if pruned > 0 {
            ...
            debug!(
                "WAL checkpoint: pruned {} entries and truncated WAL",
                pruned
            );
        }
        Ok(pruned)
    ...
    pub fn fail_outbox(&self, entry_id: i64, error: &str) -> Result<(), String> {
        ...
        if new_attempts >= self.config.max_retries {
            ...
            warn!(
                "Outbox entry {} moved to failed after {} attempts: {}",
                entry_id, new_attempts, error
            );
        } else {
            conn.execute(
            ...
            debug!(
                "Outbox entry {} retry {}/{}: {}",
                entry_id, new_attempts, self.config.max_retries, error
            );
        }
...
mod tests {
    ...
    fn test_wal_op_type_roundtrip() {
        assert_eq!(
            WalOpType::from_str(WalOpType::Write.as_str()),
            WalOpType::Write
        );
        assert_eq!(
            WalOpType::from_str(WalOpType::Delete.as_str()),
            WalOpType::Delete
        );
        assert_eq!(
            WalOpType::from_str(WalOpType::Append.as_str()),
            WalOpType::Append
        );
    }
    ...
    fn test_outbox_status_roundtrip() {
        assert_eq!(
            OutboxStatus::from_str(OutboxStatus::Pending.as_str()),
            OutboxStatus::Pending
        );
        assert_eq!(
            OutboxStatus::from_str(OutboxStatus::Processing.as_str()),
            OutboxStatus::Processing
        );
        assert_eq!(
            OutboxStatus::from_str(OutboxStatus::Failed.as_str()),
            OutboxStatus::Failed
        );
    }
    ...
    fn test_sync_profile_roundtrip() {
        assert_eq!(
            SyncProfile::from_str(SyncProfile::LocalOnly.as_str()),
            SyncProfile::LocalOnly
        );
        assert_eq!(
            SyncProfile::from_str(SyncProfile::LocalFirst.as_str()),
            SyncProfile::LocalFirst
        );
        assert_eq!(
            SyncProfile::from_str(SyncProfile::RemoteFirst.as_str()),
            SyncProfile::RemoteFirst
        );
        assert_eq!(
            SyncProfile::from_str(SyncProfile::RemoteOnly.as_str()),
