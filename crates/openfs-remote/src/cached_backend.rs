...
impl<B: Backend + Send + Sync + 'static> Backend for CachedBackend<B> {
    ...
    async fn read_with_cas_token(
        ...
        let (content, token) = self.inner.read_with_cas_token(path).await?;
        self.cache.put(path, content.clone()).await;
        Ok((content, token))
    }
    ...
    async fn compare_and_swap(
        ...
        match self.sync.mode() {
            SyncMode::WriteBack => Err(BackendError::Other(
                "compare_and_swap is not supported in write-back mode".to_string(),
            )),
            SyncMode::WriteThrough | SyncMode::None | SyncMode::PullMirror => {
                let token = self.inner.compare_and_swap(path, expected, content).await?;
                self.cache.put(path, content.to_vec()).await;
                Ok(token)
            }
        }
        ...
        match self.sync.mode() {
            ...
            SyncMode::WriteBack => {
                ...
                if let Err(e) = self.sync.queue_write(path.to_string(), new_content).await {
                    tracing::warn!(
                        "Failed to queue append for {}: {}. Data is cached but may not sync.",
                        path,
                        e
                    );
                }
    ...
    async fn delete(&self, path: &str) -> Result<(), BackendError> {
        ...
        match self.sync.mode() {
            SyncMode::WriteBack => {
                ...
                let locally_present =
                    self.cache.contains(path).await || self.sync.pending_contains(path).await;
    ...
    async fn stat(&self, path: &str) -> Result<Entry, BackendError> {
        if self.sync.mode() == SyncMode::WriteBack {
            ...
            if !normalized.is_empty() {
                ...
                if self.cache.has_prefix(&dir_prefix).await {
                    let name = normalized.rsplit('/').next().unwrap_or(normalized);
                    return Ok(Entry::dir(normalized.to_string(), name.to_string(), None));
                }
            }
        }
...
mod tests {
    ...
    async fn test_cached_backend_warm() {
        ...
        let warmed = cached
            .warm(&["/a.txt", "/b.txt", "/nonexistent.txt"])
            .await
            .unwrap();
        assert_eq!(warmed, 2);
    ...
    #[tokio::test]
    async fn test_cached_backend_compare_and_swap_write_through() {
        let inner = MemoryBackend::new();
        inner.write("/cas.txt", b"v1").await.unwrap();
        let cached = CachedBackend::write_through(inner, CacheConfig::default());

        let (_content, token) = cached.read_with_cas_token("/cas.txt").await.unwrap();
    ...
    #[tokio::test]
    async fn test_cached_backend_compare_and_swap_write_back_unsupported() {
        let inner = MemoryBackend::new();
        let cached = CachedBackend::write_back(inner, CacheConfig::default(), 1);
        let err = cached
            .compare_and_swap("/cas.txt", None, b"v1")
            .await
            .unwrap_err();
