...
impl Default for PostgresConfig {
    fn default() -> Self {
        PostgresConfig {
            ...
            table_name: "openfs_files".to_string(),
            max_connections: 5,
        }
    }
}
...
impl Backend for PostgresBackend {
    ...
    async fn read_with_cas_token(
        ...
        let path = Self::normalize_path(path);
        let query = format!(
            "SELECT content, modified FROM {} WHERE path = $1",
            self.table_name
        );
        let row: Option<(Vec<u8>, DateTime<Utc>)> = sqlx::query_as(&query)
            .bind(&path)
            .fetch_optional(&self.pool)
            .await
            .map_err(|e| BackendError::Other(format!("Postgres read failed: {}", e)))?;
        match row {
            Some((content, modified)) => Ok((content, Some(modified.to_rfc3339()))),
            None => Err(BackendError::NotFound(path)),
        }
    }
    ...
    async fn compare_and_swap(
        ...
        if let Some(expected_token) = expected {
            let expected_modified = DateTime::parse_from_rfc3339(expected_token)
                .map_err(|e| {
                    BackendError::Other(format!("Invalid CAS token '{}': {}", expected_token, e))
                })?
                .with_timezone(&Utc);

            let query = format!(
                "UPDATE {} SET content = $2, size = $3, modified = NOW() WHERE path = $1 AND modified = $4 RETURNING modified",
                self.table_name
            );
            let row: Option<(DateTime<Utc>,)> = sqlx::query_as(&query)
                ...
                .await
                .map_err(|e| {
                    BackendError::Other(format!("Postgres compare-and-swap failed: {}", e))
                })?;

            if let Some((modified,)) = row {
                return Ok(Some(modified.to_rfc3339()));
            }

            let actual = match self.read_with_cas_token(&path).await {
                Ok((_bytes, token)) => token.unwrap_or_else(|| "unknown".to_string()),
                Err(BackendError::NotFound(_)) => "absent".to_string(),
        ...
        let (modified,): (DateTime<Utc>,) = sqlx::query_as(&query)
            .bind(&path)
            .bind(content)
            .bind(size)
            .fetch_one(&self.pool)
            .await
            .map_err(|e| BackendError::Other(format!("Postgres compare-and-swap failed: {}", e)))?;

        Ok(Some(modified.to_rfc3339()))
    }
...
mod tests {
    ...
    fn test_valid_table_names() {
        assert!(validate_table_name("openfs_files").is_ok());
