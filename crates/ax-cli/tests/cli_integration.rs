...
"#,
    ...
    let config_path = temp_dir.path().join("ax.yaml");
    fs::write(&config_path, &config_content).unwrap();

    config_path.to_str().unwrap().to_string()
}

/// Get path to the ax binary.
fn ax_binary() -> String {
...
fn test_cli_write_and_cat() {
    ...
    // Write a file
    let output = Command::new(ax_binary())
        .args(["--config", &config_path, "write", "/workspace/test.txt", "hello world"])
        .output()
    ...
    assert!(output.status.success(), "cat failed: {:?}", String::from_utf8_lossy(&output.stderr));
...
fn test_cli_ls() {
    ...
    // Write some files
    Command::new(ax_binary())
        .args(["--config", &config_path, "write", "/workspace/file1.txt", "content1"])
        .output()
        .expect("Failed to execute command");

    Command::new(ax_binary())
        .args(["--config", &config_path, "write", "/workspace/file2.txt", "content2"])
        .output()
        .expect("Failed to execute command");
...
fn test_cli_rm() {
    ...
    // Write a file
    let write_output = Command::new(ax_binary())
        .args(["--config", &config_path, "write", "/workspace/to_delete.txt", "delete me"])
        .output()
        .expect("Failed to execute command");

    assert!(write_output.status.success(), "write failed: stdout={:?} stderr={:?}",
        String::from_utf8_lossy(&write_output.stdout),
        String::from_utf8_lossy(&write_output.stderr));

    // Verify it exists
    let exists_output = Command::new(ax_binary())
        .args(["--config", &config_path, "exists", "/workspace/to_delete.txt"])
        .output()
        .expect("Failed to execute command");

    assert!(exists_output.status.success(), "exists check failed: stdout={:?} stderr={:?}",
    ...
    assert!(exists_stdout.contains("exists"), "Expected 'exists' in output: {}", exists_stdout);

    // Delete it
    let rm_output = Command::new(ax_binary())
        .args(["--config", &config_path, "rm", "/workspace/to_delete.txt"])
        .output()
        .expect("Failed to execute command");

    assert!(rm_output.status.success(), "rm failed: stdout={:?} stderr={:?}",
    ...
    let final_output = Command::new(ax_binary())
        .args(["--config", &config_path, "exists", "/workspace/to_delete.txt"])
        .output()
        .expect("Failed to execute command");

    // Exit code 1 means file doesn't exist, which is what we expect
    assert!(!final_output.status.success(), "Expected file to NOT exist after deletion");
    let final_stdout = String::from_utf8_lossy(&final_output.stdout);
    assert!(final_stdout.contains("does not exist"), "Expected 'does not exist' in output: {}", final_stdout);
}
...
fn test_cli_stat() {
    ...
    // Write a file
    Command::new(ax_binary())
        .args(["--config", &config_path, "write", "/workspace/stat_test.txt", "some content"])
        .output()
        .expect("Failed to execute command");
    ...
    assert!(output.status.success(), "stat failed: {:?}", String::from_utf8_lossy(&output.stderr));
    let stdout = String::from_utf8_lossy(&output.stdout);
...
fn test_cli_tree() {
    ...
    Command::new(ax_binary())
        .args(["--config", &config_path, "write", "/workspace/a/b/c.txt", "nested"])
        .output()
        .expect("Failed to execute command");

    Command::new(ax_binary())
        .args(["--config", &config_path, "write", "/workspace/root.txt", "root"])
        .output()
        .expect("Failed to execute command");
    ...
    assert!(output.status.success(), "tree failed: {:?}", String::from_utf8_lossy(&output.stderr));
    let stdout = String::from_utf8_lossy(&output.stdout);
...
fn test_cli_tools() {
    ...
    assert!(output.status.success(), "tools failed: {:?}", String::from_utf8_lossy(&output.stderr));
    let stdout = String::from_utf8_lossy(&output.stdout);
...
fn test_cli_append() {
    ...
    Command::new(ax_binary())
        .args(["--config", &config_path, "write", "/workspace/append.txt", "hello"])
        .output()
        .expect("Failed to execute command");

    // Append more content
    let output = Command::new(ax_binary())
        .args(["--config", &config_path, "append", "/workspace/append.txt", " world"])
        .output()
        .expect("Failed to execute command");
...
fn test_cli_cp() {
    ...
    // Write a file
    Command::new(ax_binary())
        .args(["--config", &config_path, "write", "/workspace/source.txt", "copy me"])
        .output()
        .expect("Failed to execute command");

    // Copy it
    let output = Command::new(ax_binary())
        .args(["--config", &config_path, "cp", "/workspace/source.txt", "/workspace/dest.txt"])
        .output()
        .expect("Failed to execute command");
...
fn test_cli_mv() {
    ...
    // Write a file
    Command::new(ax_binary())
        .args(["--config", &config_path, "write", "/workspace/to_move.txt", "move me"])
        .output()
        .expect("Failed to execute command");

    // Move it
    let output = Command::new(ax_binary())
        .args(["--config", &config_path, "mv", "/workspace/to_move.txt", "/workspace/moved.txt"])
        .output()
        .expect("Failed to execute command");

    assert!(output.status.success(), "mv failed: {:?}", String::from_utf8_lossy(&output.stderr));
...
fn test_cli_nested_directories() {
    ...
    let output = Command::new(ax_binary())
        .args(["--config", &config_path, "write", "/workspace/a/b/c/deep.txt", "deep content"])
        .output()
        .expect("Failed to execute command");
    ...
    assert!(output.status.success());
    assert_eq!(String::from_utf8_lossy(&output.stdout).trim(), "deep content");
...
fn test_cli_find() {
    ...
    // Create some files
    Command::new(ax_binary())
        .args(["--config", &config_path, "write", "/workspace/test1.txt", "content"])
        .output()
        .expect("Failed to execute command");

    Command::new(ax_binary())
        .args(["--config", &config_path, "write", "/workspace/test2.txt", "content"])
        .output()
        .expect("Failed to execute command");

    Command::new(ax_binary())
        .args(["--config", &config_path, "write", "/workspace/other.md", "content"])
        .output()
        .expect("Failed to execute command");

    // Find txt files using regex pattern
    let output = Command::new(ax_binary())
        .args(["--config", &config_path, "find", r"\.txt$", "-p", "/workspace"])
        .output()
        .expect("Failed to execute command");

    assert!(output.status.success(), "find failed: {:?}", String::from_utf8_lossy(&output.stderr));
    let stdout = String::from_utf8_lossy(&output.stdout);
...
fn test_cli_grep() {
    ...
    Command::new(ax_binary())
        .args(["--config", &config_path, "write", "/workspace/has_match.txt", "foo bar baz"])
        .output()
        .expect("Failed to execute command");

    Command::new(ax_binary())
        .args(["--config", &config_path, "write", "/workspace/no_match.txt", "nothing here"])
        .output()
        .expect("Failed to execute command");

    // Search for pattern in specific file
    let output = Command::new(ax_binary())
        .args(["--config", &config_path, "grep", "bar", "/workspace/has_match.txt"])
        .output()
        .expect("Failed to execute command");
...
fn test_cli_overwrite_file() {
    ...
    // Write initial content
    Command::new(ax_binary())
        .args(["--config", &config_path, "write", "/workspace/overwrite.txt", "original"])
        .output()
        .expect("Failed to execute command");

    // Overwrite with new content
    Command::new(ax_binary())
        .args(["--config", &config_path, "write", "/workspace/overwrite.txt", "modified"])
        .output()
        .expect("Failed to execute command");
...
fn test_cli_unicode_content() {
    ...
    let output = Command::new(ax_binary())
        .args(["--config", &config_path, "write", "/workspace/unicode.txt", unicode_content])
        .output()
        .expect("Failed to execute command");
    ...
    assert!(output.status.success());
    assert_eq!(String::from_utf8_lossy(&output.stdout).trim(), unicode_content);
}
...
fn test_cli_empty_file() {
    ...
    // Write empty content
    let output = Command::new(ax_binary())
        .args(["--config", &config_path, "write", "/workspace/empty.txt", ""])
        .output()
        .expect("Failed to execute command");
...
fn test_cli_special_characters_in_filename() {
    ...
    let output = Command::new(ax_binary())
        .args(["--config", &config_path, "write", "/workspace/file-with_special.chars.txt", "content"])
        .output()
        .expect("Failed to execute command");
    ...
    // Read it back
    let output = Command::new(ax_binary())
        .args(["--config", &config_path, "cat", "/workspace/file-with_special.chars.txt"])
        .output()
        .expect("Failed to execute command");
...
fn test_cli_multiple_files_in_directory() {
    ...
    for i in 0..10 {
        Command::new(ax_binary())
            .args(["--config", &config_path, "write", &format!("/workspace/file_{}.txt", i), &format!("content_{}", i)])
            .output()
            .expect("Failed to execute command");
    }
...
fn test_cli_cat_nonexistent() {
    ...
    let output = Command::new(ax_binary())
        .args(["--config", &config_path, "cat", "/workspace/does_not_exist.txt"])
        .output()
        .expect("Failed to execute command");
