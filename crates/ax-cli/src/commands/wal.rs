...
pub async fn run_checkpoint(config_dir: Option<PathBuf>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    eprintln!("WAL checkpoint complete: {} total entries pruned", total_pruned);
    Ok(())
}
...
fn resolve_wal_dir(config_dir: Option<PathBuf>) -> Result<PathBuf, Box<dyn std::error::Error>> {
    ...
    if let Ok(dir) = std::env::var("AX_WAL_DIR") {
...
fn resolve_wal_paths(config_dir: Option<PathBuf>) -> Result<Vec<PathBuf>, Box<dyn std::error::Error>> {
